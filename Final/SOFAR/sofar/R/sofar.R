...
#' @export
sofar.modstr <- function(mu = 1, mugamma = 1.1,
                             WA = NULL, WB = NULL, Wd = NULL,
                             wgamma = 2) {
  list(#nrank = nrank,
    mu = mu, mugamma = mugamma,
    WA = WA, WB = WB, Wd = Wd,
    wgamma = wgamma)
}
...
#' @export
sofar.control <- function(lamA = NULL, lamB = NULL, lamD = NULL,
                              methodA = "adlasso",
                              methodB = "adlasso",
                              epsilon = 1e-3, maxit = 200L,
                              innerEpsilon = 1e-3, innerMaxit = 50L,
                              sv.tol = 1e-02) {
  list(lamA = lamA, lamB = lamB, lamD = lamD,
       methodA = methodA, methodB = methodB,
       epsilon = epsilon, maxit = maxit,
       innerEpsilon = innerEpsilon, innerMaxit = innerMaxit,
       sv.tol = sv.tol)
}
...
sofar.init <- function(Y,X,
                       nrank = 1,
                       seed = NULL
                       #ini.type = c("c","xc")[2]
                       #U=NULL,V=NULL,D=NULL,
                       #nrank=3, conv=1e-3, maxit=100,inner.conv=1e-3,inner.iter=50,
                       #methodA=c("lasso","adlasso","glasso","adglasso")[2], 
                       #methodB=c("lasso","adlasso","glasso","adglasso")[2]
){
  ##require(glmnet)
  if(is.null(seed)) seed <- 723
  set.seed(seed)
  ...
  #                           nu = nrank, 
  #                           nv = nrank)
  #   coef.rrr.reduced <- coef.svd.reduced$u%*%diag(coef.svd.reduced$d[1:nrank],nrow=nrank)%*%t(coef.svd.reduced$v)
  # }
  ...
  list(p.index = p.index,
       q.index = q.index
       # coef.lasso = coef.lasso.reduced,
       # coef.rrr = coef.rrr.reduced,
       # coef.svd = list(U=coef.svd.reduced$u,V=coef.svd.reduced$v,D=coef.svd.reduced$d[1:nrank])
  )
...
sofar.procrustes <- function(XY, XX, D, rho2, 
                            U = NULL, identity = FALSE,
  ...
  list(U=U,diff=diff,niter=niter,converged = converged)
  
}


###Fit SoFAR with fixed tuning parameters
sofar.fit <- function(Y, X,
                      XX = NULL, XY = NULL, Yvec = NULL, eigenXX = NULL,
                          nrank = 1,
                          modstr = list(),
  ...
  ## modstr
  #nrank <- modstr$nrank; 
  mu <- modstr$mu; mugamma <- modstr$mugamma
  WA <- modstr$WA; WB <- modstr$WB; Wd <- modstr$Wd
  wgamma <- modstr$wgamma
  ## control
  lamA <- control$lamA; lamB <- control$lamB; lamD <- control$lamD;
  methodA <- control$methodA; methodB <- control$methodB;
  epsilon <- control$epsilon; maxit <- control$maxit;
  innerEpsilon <- control$innerEpsilon; innerMaxit <- control$innerMaxit;
  tol <- control$sv.tol
  ...
  ## FIXME: diag(D,nrow=length(D),ncol=length(D)) need to have nrow specified for security
  ## diag(0.9) returns a matrix of 0 by 0 instead of a 1 by 1 matrix of 0.9
  A <- t(t(U) * D)  # U%*%D
  B <- t(t(V) * D)  # V%*%D
  GA <- matrix(0., p, nrank)
  GB <- matrix(0., q, nrank)
  ...
  if (is.null(WA) || is.null(WB) || is.null(Wd)){
    if (is.null(ini)) ini <- rrr.fit(Y, X, nrank, coefSVD=TRUE)        
    WA <- switch(methodA,
                 lasso    = matrix(1., p, nrank),
                 adlasso  = (ini$coefSVD$u %*% diag(ini$coefSVD$d,nrow=length(ini$coefSVD$d))) ^ (-wgamma),
                 glasso   = rep(1., p),
                 adglasso = sqrt(rowSums((ini$coefSVD$u %*% diag(ini$coefSVD$d,nrow=length(ini$coefSVD$d)))^2)) ^ (-wgamma)
    )
    WB <- switch(methodB,
                 lasso    = matrix(1., q, nrank),
                 adlasso  = (ini$coefSVD$v %*% diag(ini$coefSVD$d,nrow=length(ini$coefSVD$d))) ^ (-wgamma),
                 glasso   = rep(1., q),
                 adglasso = sqrt(rowSums((ini$coefSVD$v %*% diag(ini$coefSVD$d,nrow=length(ini$coefSVD$d)))^2)) ^ (-wgamma)
    )
  ...
  if(is.null(XX)) XX <- crossprod(X)    # t(X)%*%X
  if(is.null(eigenXX)) eigenXX <- eigen(XX)$values[1]
  if(is.null(XY)) XY <- crossprod(X, Y) # t(X)%*%Y
  if(is.null(Yvec)) Yvec <- as.vector(Y)
  ...
  for (i in 1:maxit) { ##    while(diff[iter] > conv & iter < maxit){
    ...
    if (length(nzid) == 0) nzid <- 1  ## JY: WHY?
    ...
    XXu <- XXmu
    Du <- D
    # Ustep <- sofar.procrustes(XYu, XXu, diag(Du,nrow=length(Du),ncol=length(Du)), 
    #                           rho2 = eigenXX + mu, U,
    #                           control = list(maxit=innerMaxit,
    #                                         epsilon = innerEpsilon))
    Ustep <- sofar_procrustes_RCpp(XYu, XXu, diag(Du,nrow=length(Du),ncol=length(Du)), 
                              rho2 = eigenXX + mu, U,
                              control = list(maxit=innerMaxit,
                                             epsilon = innerEpsilon))
    ...
    XU <- X%*%U
    for (k in 1:nrank){
      Xd1[,k] <- kron_RcppArma(V[,k,drop=FALSE], XU[,k,drop=FALSE])
      ## ek <- as.vector(rep(0., nrank))
      ## ek[k] <- 1
      Xd2[,k] <- kron_RcppArma(U[,k,drop=FALSE], ee[,k,drop=FALSE]) # ek) 
      Xd3[,k] <- kron_RcppArma(V[,k,drop=FALSE], ee[,k,drop=FALSE]) # ek)     
    }
    
    ########################### 
    Xd <- crossprod(Xd1) + mu * (crossprod(Xd2) + crossprod(Xd3))
    Yd <- crossprod(Xd1, Yvec) + mu * ( t(Xd2) %*% as.vector(t(A - GA)) + t(Xd3) %*% as.vector(t(B - GB)) )
    if (lamD == 0){
      D <- solve(Xd, Yd)
    } else {
      ...
      Ydw <- Wdminv * Yd
      D <- Wdminv * as.vector(lassoshooting(XtX=Xdw,Xty=Ydw,lambda=lamD)$coef)
    ...
    A <- switch(methodA,
                # lasso=softTH(U%*%diag(D,nrow=length(D))+GA,lamA/mu*WA,tol=tol),
                # adlasso=softTH(U%*%diag(D,nrow=length(D))+GA,lamA/mu*WA,tol=tol),
                # glasso=softrowTH(U%*%diag(D,nrow=length(D))+GA,lamA/mu*WA,tol=tol*nrank),
                # adglasso=softrowTH(U%*%diag(D,nrow=length(D))+GA,lamA/mu*WA,tol=tol*nrank),
                lasso=softTH(U%*%diag(D,nrow=length(D))+GA,lamA/mu*WA,tol=tol),
                adlasso=softTH(U%*%diag(D,nrow=length(D))+GA,lamA/mu*WA,tol=tol),
                glasso=softrowTH(U%*%diag(D,nrow=length(D))+GA,lamA/mu*WA,tol=tol*nrank)$C,
                adglasso=softrowTH(U%*%diag(D,nrow=length(D))+GA,lamA/mu*WA,tol=tol*nrank)$C
    )
    B <- switch(methodB,
                lasso=softTH(V%*%diag(D,nrow=length(D))+GB,lamB/mu*WB,tol=tol),
                adlasso=softTH(V%*%diag(D,nrow=length(D))+GB,lamB/mu*WB,tol=tol),
                glasso=softrowTH(V%*%diag(D,nrow=length(D))+GB,lamB/mu*WB,tol=tol*nrank)$C,
                adglasso=softrowTH(V%*%diag(D,nrow=length(D))+GB,lamB/mu*WB,tol=tol*nrank)$C
    )                
    
    #######################Dual step#############################
    GA <- (GA + (t(D * t(U)) - A)) / mugamma # (GA + (U %*% D - A)) / mugamma
    GB <- (GB + (t(D * t(V)) - B)) / mugamma # (GB + (V %*% D - B)) / mugamma
    
    mu <- mu * mugamma
    
    del <- sqrt(sum((A - A0)^2) + sum((B - B0)^2))
    if (del < epsilon) break
  }
  ...
  dfu0 <-  switch(methodA,
                  lasso=sum(U!=0),
                  adlasso=sum(U!=0),
                  glasso=sum(apply(U,1,function(a)sum(a^2))!=0)*nrank,
                  adglasso=sum(apply(U,1,function(a)sum(a^2))!=0)*nrank
  )
  dfv0 <-  switch(methodB,
                  lasso=sum(V!=0),
                  adlasso=sum(V!=0),
                  glasso=sum(apply(V,1,function(a)sum(a^2))!=0)*nrank,
                  adglasso=sum(apply(V,1,function(a)sum(a^2))!=0)*nrank
  )
  ...
  list(# diff=diff,iter=iter,
    ic = c(BIC=BIC, GIC=GIC, AIC=AIC, GCV=GCV),
    sse=sse, df=df, converged = converged,
    U=U, V=V, D=D, rank=nrank) 
}




sofar.lammax <- function(Y, X, 
                         U=NULL,V=NULL,D=NULL,
                         XX = NULL, XY = NULL, Yvec = NULL, eigenXX = NULL,
                         nrank = 3,
                         ...
                         methodA=c("lasso","adlasso","glasso","adglasso")[2], 
                         methodB=c("lasso","adlasso","glasso","adglasso")[2],
                         WA=NULL,WB=NULL,Wd=NULL,wgamma=2
){
  ...
  ####Adaptive weights
  if(is.null(WA) | is.null(WB) | is.null(Wd)){
    if(is.null(ini)) ini <- rrr.fit(Y,X,nrank=nrank,coefSVD = TRUE)        
    WA <- switch(methodA,
                 lasso=matrix(nrow=p,ncol=nrank,1),
                 adlasso=(ini$coefSVD$u%*%diag(ini$coefSVD$d,nrow=length(ini$coefSVD$d)))^{-wgamma},
                 glasso=rep(1,p),
                 adglasso=apply(ini$coefSVD$u%*%diag(ini$coefSVD$d,nrow=length(ini$coefSVD$d)),1,function(a)sqrt(sum(a^2)))^{-wgamma}
    )
    WB <- switch(methodB,
                 lasso=matrix(nrow=q,ncol=nrank,1),
                 adlasso=(ini$coefSVD$v%*%diag(ini$coefSVD$d,nrow=length(ini$coefSVD$d)))^{-wgamma},
                 glasso=rep(1,q),
                 adglasso=apply(ini$coefSVD$v%*%diag(ini$coefSVD$d,nrow=length(ini$coefSVD$d)),1,function(a)sqrt(sum(a^2)))^{-wgamma}
    )
    if(substr(methodA,1,2)=="ad" | substr(methodB,1,2)=="ad"){
      if(is.null(ini)) ini <- rrr.fit(Y,X,nrank=nrank,coefSVD = TRUE)        
      Wd <- ini$coefSVD$d^{-wgamma}
  ...
  while(diff[iter] > conv & iter < maxit){
    ...
    XXu <- XXmu
    Du <- D
    # Ustep <- sofar.procrustes(XYu, XXu, D, 
    #                           rho2 = eigenXX + mu, U,
    #                           control = list(maxit=50,
    #                                         epsilon = 1e-3))
    Ustep <- sofar_procrustes_RCpp(XYu, XXu, D, 
                                   rho2 = eigenXX + mu, U,
                                   control = list(maxit=50,
                                                  epsilon = 1e-3))
    U <- Ustep$U
    ...
    Xd <- t(Xd1)%*%Xd1 + mu*(t(Xd2)%*%Xd2+t(Xd3)%*%Xd3+diag(nrow=nrank))
    Yd <- t(Xd1)%*%Yvec + mu*(t(Xd2)%*%as.vector(t(A-GA))+t(Xd3)%*%as.vector(t(B-GB))+as.vector(diag(O-GO)))
    d <- solve(Xd)%*%Yd
    d[d<tol] <- 0
    ...
    ##Estimate lamA.max
    lamA.max <- switch(methodA,
                       lasso=max(abs((U%*%D+GA)*mu/WA)),
                       adlasso=max(abs((U%*%D+GA)*mu/WA)),
                       glasso=max(abs(lnorm(U%*%D+GA,2)*mu/WA)),
                       adglasso=max(abs(lnorm(U%*%D+GA,2)*mu/WA))
    )            
    B <- switch(methodB,
    ...
    lamB.max <- switch(methodB,
                       lasso=max(abs((V%*%D+GB)*mu/WB)),
                       adlasso=max(abs((V%*%D+GB)*mu/WB)),
                       glasso=max(abs(lnorm(V%*%D+GB,2)*mu/WB)),
                       adglasso=max(abs(lnorm(V%*%D+GB,2)*mu/WB))
    ) 
    ...
    diff[iter] <- sqrt(sum((A-A0)^2)+sum((B-B0)^2)+sum((O-O0)^2))
  }
  
  list(#diff=diff,iter=iter,BIC=BIC,GIC=GIC,AIC=AIC,GCV=GCV,
    #sse=sse,df=df,conv_flag=conv_flag,U=U,V=V,D=D,
    lamA.max=lamA.max,
    lamB.max=lamB.max,
    lamD.max=lamD.max)
...
sofar.path.reduced <- function(Y, X,
                               XX = NULL, XY = NULL, Yvec = NULL, eigenXX = NULL,
                               nrank = 1,
                               lamA=1, lamB=1, lamD=1,
  ...
  ini <- NULL
  ## Adaptive weights
  WA <- modstr$WA; WB <- modstr$WB; Wd <- modstr$Wd; wgamma <- modstr$wgamma
  methodA <- control$methodA; methodB <- control$methodB;
  if (is.null(WA) || is.null(WB) || is.null(Wd)){
    if (is.null(ini)) ini <- rrr.fit(Y, X, nrank, coefSVD=TRUE)        
    WA <- switch(methodA,
  ...
  for (h in 1:nlam) {
    ...
    #modstr$nrank <- rfit
    modstr$WA <- WA1; modstr$WB <- WB1; modstr$Wd <- Wd1
    control$lamA <- lamA[h]; control$lamB <- lamB[h]; control$lamD <- lamD[h]
    
    ###Use warm start
    fit <- sofar.fit(Y, X,
                     XX = XX, XY = XY, Yvec = Yvec, eigenXX = eigenXX,
                         nrank  = rfit,
                         modstr = modstr,
    ...
    if (rfit == 0) {
      ICpath[h:nlam, ] <- matrix(fit$ic, nrow=nlam - h + 1, ncol=4, byrow=TRUE)
      Rpath[h:nlam] <- 0
      break
    }
    ICpath[h,] <- fit$ic
  ...
  ic <- match.arg(ic, c("BIC", "GIC", "AIC", "GCV"))
  IC <- match(ic, c("BIC", "GIC", "AIC", "GCV"))
  ...
  list(Upath=Upath,Vpath=Vpath,Dpath=Dpath,Rpath=Rpath,ICpath=ICpath,
       lam.id = minidx,
...
sofar.cv <- function(Y, X, 
                     XX = NULL, XY = NULL, Yvec = NULL, eigenXX = eigenXX,
                     nrank = 1,
                         lamA = 1, lamB = 1, lamD = 1,
                         nfold = 5, norder = NULL, #fold.drop = 0,
                         modstr = list(),
                         init = list(U = NULL, V = NULL, D = NULL),
                         control = list()
) {
  ...
  if(is.null(init$U) || is.null(init$V) || is.null(init$D)){
    init <- list(U = ini$coefSVD$u, V = ini$coefSVD$v, D= ini$coefSVD$d)
  }
  ...
  nlam <- length(lamA)
  cr_path <- array(dim=c(nlam,nfold),NA)
  ...
  for(f in 1:nfold){
    ...
    fitf <- sofar.path.reduced(Yf,Xf,
                              XX = XXf, XY = XYf, Yvec = Yvecf, eigenXX = eigenXXf,
                              nrank = nrank,
                              lamA=lamA,lamB=lamB,lamD=lamD,
                              ic="GIC",
                              modstr = modstr,
                              control = control,
                              init = init)
    ...
    for(ll in 1:nlam){
      cr_path[ll,f] <- sum((Yfdel-Xfdel%*%fitf$Upath[ll,,]%*%diag(fitf$Dpath[ll,],nrow=nrank)%*%t(fitf$Vpath[ll,,]))^2)    
    }
    
  }
  
  index <- order(apply(cr_path,2,sum))#[1:ifelse(nfold>5,nfold-fold.drop,nfold)]    
  crerr <- apply(cr_path[,index],1,sum)/length(index)*nfold
  minid <- which.min(crerr)
  
  ###Refit model with all data.
  ##modstr$WA <- WA; modstr$WB <- WB; modstr$Wd <- Wd
  control$lamA <- lamA[minid]; control$lamB <- lamB[minid]; control$lamD <- lamD[minid]
  ##Initial values
  ##init <- list(U=fitf$Upath[minid,,],V=fitf$Vpath[minid,,],D=fitf$Dpath[minid,])
  fitall <- sofar.path.reduced(Y, X, 
                            XX = XX, XY = XY, Yvec = Yvec, eigenXX = eigenXX,
                            ic = "GIC",
                            nrank = nrank,
                            lamA=lamA,lamB=lamB,lamD=lamD,
...
#' @param lam.min.factor set the smallest lambda triplets as a fraction of the estimation lambda.max triplets
#' @param lam.max.factor set the largest lambda triplets as a multiple of the estimation lambda.max triplets
#' @param lam.AB.factor set the relative penalty level between A/B and D
#' @param penA,penB,penD if TRUE, penalty is applied
#' @param tuning select tuning method; the default is GIC
#' @param nfold,norder,seed used if tuning = "CV"
#' @param modstr a list of internal model parameters controlling the model fitting
#' @param control a list of internal computation parameters controlling optimization
#' @aliases sofar
#' @return 
...
#' X <- mydata$X
#' 
#' fit1 <- sofar.path(Y,X, tuning = "GIC", nrank=nrank+2, 
...
sofar.path <- function(Y, X,
                       nrank = 1, nlam = 50,
                       lam.min.factor = 1e-4, lam.max.factor = 10,
                       lam.AB.factor = 1,
                       penA = TRUE, penB = TRUE, penD = TRUE,
                       tuning = c("CV","BIC","GIC","AIC","GCV")[3],
                       nfold = 5, norder = NULL, seed = NULL, #if tuning = "CV"
                       modstr = list(),
                       control = list()
) {
  ...
  ##Load model parameters and controls 
  modstr <- do.call("sofar.modstr", modstr)
  control <- do.call("sofar.control", control)
  mu <- modstr$mu; mugamma <- modstr$mugamma
  WA <- modstr$WA; WB <- modstr$WB; Wd <- modstr$Wd
  ...
  ini.screen <- sofar.init(Y,X, nrank, seed = seed)
  ...
  ini <- NULL
  if (is.null(WA) || is.null(WB) || is.null(Wd)){
    if (is.null(ini)) ini <- rrr.fit(Y.reduced, X.reduced, nrank, coefSVD=TRUE)        
    WA <- switch(methodA,
  ...
  if(is.null(lamA) || is.null(lamB) || is.null(lamD)){  
    lammax <- sofar.lammax(Y.reduced, X.reduced, 
                      U=U,V=V,D=diag(D,nrow=nrank),
                      nrank = nrank,
                      mu=mu,
                      mugamma=mugamma,
                      conv=1e-4, maxit=maxit,
                      methodA=methodA, 
                      methodB=methodB,
                      WA=WA,WB=WB,Wd=Wd,wgamma=2
    )
    lamA.max <- lammax$lamA.max
    lamB.max <- lammax$lamB.max        
    lamD.max <- lammax$lamD.max
    if(penA){
      ##lamA <- c(0,exp(seq(log(lamA.max*lam.min.factor),log(lamA.max*lam.max.factor),length=nlam)))
      lamA <- exp(seq(log(lamA.max*lam.min.factor),log(lamA.max*lam.max.factor),length=nlam))
    }else{
      ##lamA <- rep(0,nlam + 1)
      lamA <- rep(0,nlam)
    }
    if(penB){
      ##lamB <- c(0,exp(seq(log(lamB.max*lam.min.factor),log(lamB.max*lam.max.factor),length=nlam)))   
      lamB <- exp(seq(log(lamB.max*lam.min.factor*lam.AB.factor),log(lamB.max*lam.max.factor*lam.AB.factor),length=nlam))
    }else{
      #lamB <- rep(0, nlam + 1)
      lamB <- rep(0, nlam)
    }  
    if(penD){
      #lamD <- c(0,exp(seq(log(lamD.max*lam.min.factor),log(lamD.max*lam.max.factor),length=nlam)))   
      lamD <- exp(seq(log(lamD.max*lam.min.factor),log(lamD.max*lam.max.factor),length=nlam))
    }else{
      #lamD <- rep(0,nlam + 1)
      lamD <- rep(0,nlam)
    }
   ...
   XX.reduced <- crossprod(X.reduced)    # t(X)%*%X
   eigenXX.reduced <- eigen(XX.reduced)$values[1]
   XY.reduced <- crossprod(X.reduced, Y.reduced) # t(X)%*%Y
   Yvec.reduced <- as.vector(Y.reduced)
    
  if(tuning == "CV"){
    fit <- sofar.cv(Y.reduced, X.reduced, 
                    XX = XX.reduced, XY = XY.reduced, Yvec = Yvec.reduced, eigenXX = eigenXX.reduced,
                    nrank = nrank, nfold = nfold, norder = norder,
                    lamA=lamA,lamB=lamB,lamD=lamD,
                    modstr = modstr,
                    init = init,
                    control = control) 
  }else{
    fit <- sofar.path.reduced(Y.reduced, X.reduced, 
                              XX = XX.reduced, XY = XY.reduced, Yvec = Yvec.reduced, eigenXX = eigenXX.reduced,
                              ic = tuning,
                              nrank = nrank,
                              lamA=lamA,lamB=lamB,lamD=lamD,
