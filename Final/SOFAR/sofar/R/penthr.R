...
softrowTH <- function(C, Lam, tol=1e-4) {
  l2row <- sqrt(rowSums(C^2)) # apply(C, 1, function(x) sqrt(sum(x^2)))
  under <- l2row - Lam < tol
  C[under, ] <- 0
...
svdThres <- function(C, maxrank = min(dim(C)), threshold = hardThres,
                     lambda = 0) {
  svdC <- svd(C, nu = maxrank, nv = maxrank)
  d <- threshold(svdC$d, lambda)
  rank <- sum(d!=0)
  list(C = svdC$u %*% diag(d, nrow=maxrank, ncol=maxrank) %*% t(svdC$v),
       rank = rank)
}

## singular value penalty
svdPen <- function(C, maxrank = min(dim(C)), penalty = softPen,
                   lambda = 0) {
  svdC <- svd(C, nu = maxrank, nv = maxrank)
