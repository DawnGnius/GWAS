...
with p-value less than a specific 0.05. 
```{r}
rm(list=ls())
DEBUG <- FALSE
load('tmp.RData')
...
# Estimation
res.sofar <- sofar(Y, X, nrank=3, ic.type="GIC", control=list(lamA=0.1, lamB=0.1, lamD=0.1))
...
heatmap_plot <- function(model_fit) {
  ...
  p1 <-
    ...
    scale_fill_gradient2(low = "red",
                        ...
                        ) +theme(panel.background = element_rect(fill = 'white', colour = 'white'),plot.title = element_text(hjust = 0.5),legend.position = "none",axis.text.x = element_text(
                           angle = 90,
                           vjust = 0,
                           size = 4.5,
                           hjust = .5
                         ),axis.title.x=element_blank(),axis.title.y=element_blank())+ggtitle("Real")
p2 <-
  ggplot(df, aes(Var1, Var2, fill = fill2)) + geom_tile() +
  scale_fill_gradient2(low = "red",
                       high = "blue",
                       mid = "white") + theme(panel.background = element_rect(fill = 'white', colour = 'white'),plot.title = element_text(hjust = 0.5),legend.position = "none", axis.text.x = element_text(
                         angle = 90,
                         vjust = 1,
                         size = 4.5,
                         hjust = .5
                       ),axis.title.x=element_blank(),axis.title.y=element_blank())+ggtitle("Estimated")
...
svd_plot <- function(fit_model) {
  ...
  p <- ggplot(df,
              aes(x = plot_x, y = plot_y)) + geom_point()  + 
