...
a0 <- matrix(0., ncol=ncol(Y))
for (ii in 1:dim(Y)[2]) {
  fit.cv <- glmnet::cv.glmnet(X, Y[,ii])
  fit.glmnet <- glmnet::glmnet(X, Y[,ii], lambda=fit.cv$lambda.min)
  beta.hat[, ii] <- as.matrix(fit.glmnet$beta)
  a0[1, ii] <- fit.glmnet$a0                          # with intercept
}
...
df_lasso$fill2 <- as.vector(t(X %*% beta.hat + matrix(1., nrow=nrow(X)) %*% a0))   # with intercept
...
```{r, eval=FALSE}
library(grpreg)
# lambda.group <- cv.grpreg(X, Y, )$lambda.min
res.group <- grpreg(X, Y, lambda=0.001)
beta.group <- t(res.group$beta[, , 1])

heatmap(Y, Rowv=NA, Colv=NA)
heatmap(cbind(matrix(1., nrow=nrow(X)), X) %*% beta.group, Rowv=NA, Colv=NA)

gene_names <- colnames(Y)
sample_num <- 1:112
df_lasso <- expand.grid(gene_names, sample_num)
df_lasso$fill1 <- as.vector(t(Y))
df_lasso$fill2 <- as.vector(t(cbind(matrix(1., nrow=nrow(X)), X) %*% beta.group))   # with intercept
p1_real <-  ggplot(df_lasso, aes(Var1, Var2, fill = fill1)) +
  geom_tile() + scale_fill_gradient2(low = "red", high = "blue") + 
  theme(panel.background = element_rect(fill = 'white', colour = 'white'),
    plot.title = element_text(hjust = 0.5),
...
p2_lasso <- ggplot(df_lasso, aes(Var1, Var2, fill = fill2)) +
  ...
  theme(panel.background = element_rect(fill = 'white', colour = 'white'),
    ...
    axis.title.x = element_blank(),
    axis.title.y = element_blank()) + 
  ggtitle("Estimated by GRPREG")
p_lasso <- ggarrange(p1_real, p2_lasso, nrow = 1)
p_lasso

```

The rank of estimated coefficient matrix $C$ is `r qr(beta.group)$rank`, it means that $\hat{C}$ is full rank. 
Also, heatmap shows that lasso loses many a detials of $Y$. 
There are `r length(which(rowSums(abs(beta.group)) != 0))` nonzero rows in $\hat{C}$. 
Of couse, this method does not satisfy the requirement to explain the correlation between $Y$. 
...
library(trigger)
data(yeast)
var.selected <- idx.X[which(u.rowsums != 0)]
marker.selected <- yeast$marker.pos[var.selected, ]
gene.selected <- MAPK.pathway.name[which(v.rowsums != 0)]
```